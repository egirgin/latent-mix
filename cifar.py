...
from latentcomp import load_nets, latent_train, random_composition
...
parser.add_argument(
    ...
    '-latent',
    action='store_true',
    help='Use latent composition while mixing clean and aug instead of convex combination.') 
...
def aug(image, preprocess, latent_nets=None):
...
class AugMixDataset(torch.utils.data.Dataset):
  ...
  def __init__(self, dataset, preprocess, no_jsd=False, latent_net=None):
  ...
  def __getitem__(self, i):
    ...
    if self.no_jsd:
      if self.latent_net:
        return aug(x, self.preprocess, nets), y
      else:
        return aug(x, self.preprocess), y
    else:
      if self.latent_net:
        im_tuple = (self.preprocess(x), aug(x, self.preprocess, nets=self.latent_net),
                  aug(x, self.preprocess, nets=self.latent_net))
      else:
